...
**2. How are PCA and Clustering Methods different?**

PCA is used to reduce the number of variables in the data to facilitate explanatory data analysis and more efficient predictive models. The original variables are transformed into principle components, a new set of variables that is not correlated and not ordered. By transforming the original variables into the new set of variables, the first few variables preserve the majority of the original variation of the initial variables. PCA identifies the main direction in which the data varies most as the first principal component; the following principal components are perpendicular to the first principal component and to all the newly created additional ones. PCA is helpful when there is multicollinearity in the data or when the data we are working with is high-dimensional.

Clustering is used to group similar data into clusters, where the data within one cluster is more similar to the other data within the cluster than to data outside of the cluster. Clustering allows for the identification of structure in the data, helps identify data with similar characteristics, or allows us to group the data according to similar characteristics. When clustering the data, a clustering algorithm assigns data to clusters based on their similarity to or distance from each other. By clustering, we discover groups in the data that we previously did not know about.

While PCA aims to maintain as much of the variability of the data as possible while reducing the dimensionality, clustering groups data together according to its similarity. PCA makes the data easier to work with, while clustering allows us to group data into meaningful groupings.
